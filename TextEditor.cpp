...
TextEditor::TextEditor()
	: mLineSpacing( 1.0f ), mUndoIndex( 0 ), mTabSize( 4 ), mOverwrite( false ), mReadOnly( false ),
	  ...
	  mShowWhitespaces( true ), mStartTime( std::chrono::duration_cast<std::chrono::milliseconds>(
		...
		if ( !IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed( ImGui::GetKeyIndex( ImGuiKey_Z ) ) ) {
			...
			if ( mCompletion ) {
				if ( mCompletionId )
					...
					mCompletionId = mCompletions.size() - 1;
			} else
				MoveUp( 1, shift );
...
void TextEditor::Render() {
	...
	if ( !mLines.empty() ) {
		...
		while ( lineNo <= lineMax ) {
			...
			longest	   = std::max(
				   mTextStart + TextDistanceToLineStart( Coordinates( lineNo, GetLineMaxColumn( lineNo ) ) ), longest );
			auto		columnNo = 0;
	...
	{ // autocompletion
		...
		if ( mCompletion ) {
			if ( !mCurrentWord.empty() && !mCompletions.empty() ) {
				...
				ImGui::SetNextWindowPos( CoordinatesToScreenPos( cursorPos ), ImGuiCond_Always );
				ImGui::SetNextWindowBgAlpha( 0.5f );
				ImGui::Begin( "AutoCompletion", nullptr, popupFlags );
				ImGui::PushAllowKeyboardFocus( false );

				for ( int i = 0; i < mCompletions.size(); i++ ) {
					ImGui::Selectable( mCompletions[i].first.c_str(), i == mCompletionId );
...
ImVec2 TextEditor::TextEditor::CoordinatesToScreenPos( const TextEditor::Coordinates &aPosition ) const {
	ImVec2 origin = mUICursorPos;
	int	   dist	  = aPosition.mColumn;

	int retY = origin.y + aPosition.mLine * mCharAdvance.y;
	int retX = origin.x + 7 * mCharAdvance.x + dist * mCharAdvance.x - ImGui::GetScrollX();
